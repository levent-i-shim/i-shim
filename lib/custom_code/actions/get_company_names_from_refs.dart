...
Future<List<String>?> getCompanyNamesFromRefs(
  List<DocumentReference>? listOfCompanyRefsFromCustomAction,
) async {
  // Null / boş kontrolü
  if (listOfCompanyRefsFromCustomAction == null ||
      listOfCompanyRefsFromCustomAction.isEmpty) {
  ...
  for (final r in listOfCompanyRefsFromCustomAction) {
    if (r == null) continue;
    final key = r.path;
    if (seen.add(key)) {
      refsInOrder.add(r);
